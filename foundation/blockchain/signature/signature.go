...
func Sign(value any, privateKey *ecdsa.PrivateKey) (v, r, s *big.Int, err error) {
	...
	// Check the public key validates the data and signatures.
...
func stamp(value any) ([]byte, error) {
	...
	// Hash the data into a 32 byte array. This will provide a data
	// length consistency with all data.
	txHash := crypto.Keccak256(v)

	// Convert the stamp into a slice of bytes. This stamp is used
	// so signatures we produce when signing data are always
...
// toSignatureValues converts the signature into r, s, v values.
